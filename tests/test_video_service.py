...
def test_video_service_hello_offer_stop_flow() -> None:
    ...
    service = VideoService(
        ...
        sender_factory=lambda _source, _session_id, _fps: fake_sender,
    )
    fake_signaling = _FakeSignaling()
    service._signaling = fake_signaling  # type: ignore[attr-defined]

    connection = _FakeConnection()
    session_id = "sess-1"

    async def _run() -> None:
        await service._on_message(  # type: ignore[attr-defined]
            connection, SignalingMessage(type="hello", session_id=session_id, payload={})
        )
        await service._on_message(  # type: ignore[attr-defined]
            connection, SignalingMessage(type="start_video", session_id=session_id, payload={})
        )
        await service._on_message(  # type: ignore[attr-defined]
            connection,
            SignalingMessage(
        ...
        await service._on_message(  # type: ignore[attr-defined]
            connection, SignalingMessage(type="stop_video", session_id=session_id, payload={})
        )
